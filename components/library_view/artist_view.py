...
class Artist_View(tk.Frame):
    def __init__(self,parent,artist,albums,size):
        ...
        if len(self.albums_from_artist)>4:
            self.albums_from_artist[4:] = []
        match len(self.albums_from_artist):
            case 1:
                cover_artist1 = self.set_images_in_places(self.albums_from_artist[0],(int(self.size),int(self.size)))
                cover_artist1.pack()
            case 2:
                covers_size = int(self.size/2)
                cover_artist1 = self.set_images_in_places(self.albums_from_artist[0],(int(covers_size),int(self.size)))
                cover_artist2 = self.set_images_in_places(self.albums_from_artist[1],(int(covers_size),int(self.size)))
                cover_artist1.pack(side='left')
                cover_artist2.pack(side='left')
            case 3:
                covers_size = int(self.size/2)
                cover_artist1 = self.set_images_in_places(self.albums_from_artist[0],(int(covers_size),int(covers_size)))
                cover_artist2 = self.set_images_in_places(self.albums_from_artist[1],(int(covers_size),int(covers_size)))
                cover_artist3 = self.set_images_in_places(self.albums_from_artist[2],(int(self.size),int(covers_size)))
            ...
            case 4:
                ...
                cover_artist1 = self.set_images_in_places(self.albums_from_artist[0],(int(covers_size),int(covers_size)))
                cover_artist2 = self.set_images_in_places(self.albums_from_artist[1],(int(covers_size),int(covers_size)))
                cover_artist3 = self.set_images_in_places(self.albums_from_artist[2],(int(covers_size),int(covers_size)))
                cover_artist4 = self.set_images_in_places(self.albums_from_artist[3],(int(covers_size),int(covers_size)))
                cover_artist1.grid(row=0,column=0)
                cover_artist2.grid(row=0,column=1)
